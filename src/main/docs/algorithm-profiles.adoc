...
* **Implementation:** `net.openhft.hashing.MurmurHash_3` adapts Austin Appleby’s x64 variants.
...
* **Factories:** `LongHashFunction.xx3()`, `.xx3(long)` for 64-bit, plus `.xx128low()` / `.xx128low(long)` for the low 64 bits of XXH128 (`LongHashFunction.java:300-341`).
Full 128-bit results live behind `LongTupleHashFunction.xx128()` and `.xx128(long)` (`LongTupleHashFunction.java:71-104`).
* **Implementation:** `net.openhft.hashing.XXH3` keeps the FARSH-derived 192 byte secret and streaming logic.
